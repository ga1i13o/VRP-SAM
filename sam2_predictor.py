...
class SAM2pred(nn.Module):
    ...
    def forward_img_encoder(self, query_img):
        ...
        with torch.no_grad():
            query_feats = self.sam_model.extract_features(query_img)
        return  query_feats
    ...
    def forward_prompt_encoder(self, points=None, boxes=None, protos=None, masks=None):
        ...
        #         protos=protos,
        #         masks=None)
        sparse_embeddings, dense_embeddings = self.sam_model.sam.sam_prompt_encoder(
            points=points,
            boxes=None,
            masks=None,
            protos=protos,
    ...
    def forward_mask_decoder(self, query_feats, sparse_embeddings, dense_embeddings, ori_size=(512,512)):
        current_vision_feats = query_feats.get_current_feats(0)
        high_res_features = query_feats.get_high_res_features(current_vision_feats)
        pix_feat_no_mem = current_vision_feats[-1:][-1] + self.sam_model.sam.no_mem_embed
        pix_feat_no_mem = pix_feat_no_mem.permute(1, 2, 0).view(1, 256, 64, 64)

        low_res_masks, iou_predictions, _, _, _, _, _ = self.sam_model.sam.sam_mask_decoder(
            image_embeddings=pix_feat_no_mem,
            image_pe=self.sam_model.sam.sam_prompt_encoder.get_dense_pe(),
